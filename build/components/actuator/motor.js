export class Motor {
    constructor(scene, reference, x, y, width, height, point1, point2, powerToSpeed) {
        ...
        const deltaPoint1 = Math.sqrt(Math.pow(point1.x, 2) + Math.pow(point1.y, 2)), deltaPoint2 = Math.sqrt(Math.pow(point2.x, 2) + Math.pow(point2.y, 2));
    ...
    update() {
        const deltaX = this.wheel.x - this.wheel.body.positionPrev.x, deltaY = this.wheel.y - this.wheel.body.positionPrev.y, rotationSpeed = Math.round((Math.sqrt(Math.pow(deltaX, 2) + Math.pow(deltaY, 2)) / this.radius) *
            (100 / 12) *
            5.6 *
            100) / 100;
