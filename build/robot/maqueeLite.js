...
export class MaqueenLite {
    constructor(scene, name, x, y, angle) {
        ...
        let speedGrowth = function (power) {
            return (-9e-9 * Math.pow(power, 4) +
                7e-6 * Math.pow(power, 3) -
                0.0021 * Math.pow(power, 2) +
                0.3121 * power -
                1.2);
        };
        this.motorLeft = new Motor(scene, this.body, -35, 18, 9, 43, { x: 0, y: 0 }, { x: 0, y: 40 }, speedGrowth);
        this.motorRight = new Motor(scene, this.body, 35, 18, 9, 43, { x: 0, y: -1 }, { x: 0, y: 40 }, speedGrowth);
