...
export class MaqueenPlus {
    constructor(scene, name, x, y, angle) {
        ...
        let speedGrowth = function (power) {
            return (-1e-8 * Math.pow(power, 4) +
                1e-5 * Math.pow(power, 3) -
                0.0032 * Math.pow(power, 2) +
                0.4053 * power -
                2.8394);
        };
        this.motorLeft = new Motor(scene, this.body, -45, 27, 9, 43, { x: -10, y: 0 }, { x: -10, y: 49 }, speedGrowth);
        this.motorRight = new Motor(scene, this.body, 45, 27, 9, 43, { x: 10, y: -9 }, { x: 10, y: 49 }, speedGrowth);
